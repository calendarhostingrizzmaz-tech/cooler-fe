...
const iconClass = 'w-[22px] h-[22px] sm:w-6 sm:h-6 text-gray-900';

const SocialRailIcon: Record<string, React.ReactNode> = {
  facebook: (
    <svg className={iconClass} viewBox="0 0 24 24" fill="currentColor" aria-hidden>
      <path d="M24 12.073C24 5.405 18.627 0 12 0S0 5.405 0 12c0 5.996 4.388 10.978 10.125 11.854v-8.385H7.078V12h3.047V9.356c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874V12h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
    </svg>
  ),
  twitter: (
    <svg className={iconClass} viewBox="0 0 24 24" fill="currentColor" aria-hidden>
...
const Footer: React.FC = () => {
  return (
    <>
      <aside
        className="fixed right-0 top-1/2 z-[90] flex -translate-y-1/2 flex-col overflow-hidden rounded-l-xl shadow-lg ring-1 ring-black/10"
        aria-label="Social media"
      >
        {socialRail.map(({ id, href, label, panel }) => (
          <a
            key={id}
            href={href}
            target="_blank"
            rel="noopener noreferrer"
            className={`flex h-11 w-11 items-center justify-center transition-opacity hover:opacity-90 active:opacity-100 sm:h-12 sm:w-12 ${panel}`}
            aria-label={label}
          >
            {SocialRailIcon[id]}
          </a>
    ...
    <footer className="bg-gray-900 text-white py-24 rounded-t-[4rem] px-4 overflow-hidden relative">
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-12 relative z-10">
        ...
        <div className="col-span-1 md:col-span-1">
          ...
          <p className="text-gray-400 font-medium leading-relaxed max-w-xs pr-4">
        ...
        <div>
          ...
          <ul className="space-y-4 text-gray-400 font-medium">
            ...
            <li><Link to="/donation" className="hover:text-blue-600 transition-all">Donate (Ehsaal-e-Sawab)</Link></li>
